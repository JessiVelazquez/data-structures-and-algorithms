...
// const functions = require('../lib/ll-zip.js');
...
describe('Linked List', () => {
  ...
  it('should zip two linked lists together so that the nodes alternate between the two lists and return a reference to the head of the zipped list', () => {
    let list1 = new LL();
    let first = 1;
    let second = 3;
    let third = 5;
    ...
    let third2 = 6;
    list2.append(first2);
    list2.append(second2);
    list2.append(third2);

    let answer = list1.zipLists(list1, list2);
    expect(answer.toString()).toEqual('{ 1 } -> { 2 } -> { 3 } -> { 4 } -> { 5 } -> { 6 } -> NULL');
  });
